...
{{#if history}}**Historial de Respuestas del Usuario (para tu análisis):** \`\`\`json {{{history}}} \`\`\` **Instrucciones para la Progresión (Basado en el Historial):** 1. **NO REPITAS PREGUNTAS** del historial. 2. **Analiza el historial:** Identifica los temas que el usuario domina (aciertos consistentes) y sus áreas débiles (errores). 3. **Crea preguntas más avanzadas:** Para los temas que el usuario domina, introduce conceptos más complejos o matices. Por ejemplo, si sabe sobre la importancia de la proteína, pregúntale sobre tipos de aminoácidos o el concepto de síntesis de proteína muscular. 4. **Refuerza las debilidades:** Si el usuario falla en preguntas sobre un músculo específico, genera una nueva pregunta sobre la función de ese músculo o un ejercicio para fortalecerlo. 5. **Introduce nuevos temas:** Asegúrate de incluir también temas que no hayan aparecido antes para ampliar su conocimiento.{{/if}}
**Formato para cada pregunta:** 1. Formula una 'question' (pregunta) clara y directa. 2. Crea un array 'options' con 4 posibles respuestas. Una debe ser la correcta y las otras 3 deben ser "distractores" plausibles pero incorrectos. 3. Establece 'correctAnswerIndex' al índice (0-3) de la respuesta correcta en el array 'options'. 4. Escribe una 'explanation' (explicación) clara y concisa que justifique la respuesta correcta. Genera el array de preguntas.`,
});
